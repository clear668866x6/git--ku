...
class Parser:
    ...
    def parse_func_def(self):
        ...
        if self.cur() and self.cur().value == ';':
            self.eat('PUNCT', ';')
            return {'type': 'FuncDecl', 'name': name, 'ret': ret_type, 'params': params}
        body = self.parse_block()
        return {'type': 'FuncDef', 'name': name, 'ret': ret_type, 'params': params, 'body': body}
    ...
    def parse_func_def_no_type(self):
        ...
        if self.cur() and self.cur().value == ';':
            self.eat('PUNCT', ';')
            return {'type': 'FuncDecl', 'name': name, 'ret': 'int', 'params': params}
        body = self.parse_block()
        return {'type': 'FuncDef', 'name': name, 'ret': 'int', 'params': params, 'body': body}
    ...
    def parse_var_decl(self):
        ...
        while True:
            ...
            decls.append({'name': name, 'arr_size': arr_size, 'init': init})
            if self.cur() and self.cur().value == ',':
                self.eat('PUNCT', ',')
            else:
                break
        self.eat('PUNCT', ';')
        return {'type': 'VarDecl', 'decls': decls}
    ...
    def parse_if(self):
        ...
        return {'type': 'If', 'cond': cond, 'true': tb, 'false': fb}
    ...
    def parse_do_while(self):
        ...
        self.eat('PUNCT', ')')
        self.eat('PUNCT', ';')
        return {'type': 'DoWhile', 'cond': cond, 'body': body}
    ...
    def parse_assign(self):
        ...
        if self.cur() and self.cur().value == '=':
            ...
            return {'type': 'Assign', 'target': node, 'expr': right}
        return node
    ...
    def parse_postfix(self):
        ...
        while True:
            ...
            if t and t.value == '[':
                ...
                idx = self.parse_expr()
                self.eat('PUNCT', ']')
                node = {'type': 'ArrayAccess', 'array': node, 'index': idx}
            elif t and t.value == '(':
                self.eat('PUNCT', '(')
                args = []
                while self.cur() and self.cur().value != ')':
                ...
                op = self.eat('OP').value
                node = {'type': 'PostOp', 'op': op, 'operand': node}
            else:
                break
        return node
    ...
    def parse_primary(self):
        ...
        if t.type == 'NUM':
            self.eat('NUM')
            return {'type': 'Num', 'value': int(t.value)}
        elif t.type == 'STRING':
            self.eat('STRING')
            val_str = t.value
...
def ast_to_str(node, indent=0):
    ...
    if t == 'Program':
        ...
        params_str = ', '.join(f"{p['type']} {p['name']}" for p in node['params'])
        lines.append(f"{prefix}FunctionDecl({node['name']}({params_str}));")
    elif t == 'Block':
        if indent == 0:
            lines.append(f"{prefix}Compound")
        for s in node['body']:
            lines.append(ast_to_str(s, indent + 1 if indent == 0 else indent))
    elif t == 'FuncDef':
        params_str = ', '.join(f"{p['type']} {p['name']}" for p in node['params'])
        lines.append(f"{prefix}FunctionDecl({node['name']}({params_str}))")
        ...
        for d in node['decls']:
            lines.append(f"{prefix}DeclStmt(type=int)")
            if d['arr_size']:
                lines.append(f"{prefix}  DeclItem({d['name']}[{d['arr_size']}])")
            else:
                lines.append(f"{prefix}  DeclItem({d['name']})")
    elif t == 'Assign':
        target_str = ast_to_str(node['target'], 0).strip()
        lines.append(f"{prefix}AssignStmt({target_str})")
        lines.append(f"{prefix}  Expr({ast_to_str(node['expr'], 0).strip()})")
    elif t == 'If':
        lines.append(f"{prefix}IfStmt")
        lines.append(f"{prefix}  Condition({ast_to_str(node['cond'], 0).strip()})")
        ...
        if node['init']:
            lines.append(f"{prefix}  Init({ast_to_str(node['init'], 0).strip()})")
        if node['cond']:
            lines.append(f"{prefix}  Condition({ast_to_str(node['cond'], 0).strip()})")
        if node['step']:
            lines.append(f"{prefix}  Step({ast_to_str(node['step'], 0).strip()})")
        lines.append(ast_to_str(node['body'], indent + 1))
    elif t == 'DoWhile':
        ...
        lines.append(f"{prefix}ContinueStmt")
    elif t == 'Return':
        if node['expr']:
            lines.append(f"{prefix}ReturnStmt({ast_to_str(node['expr'], 0).strip()})")
        else:
            lines.append(f"{prefix}ReturnStmt")
    elif t == 'Write':
        for a in node['args']:
            lines.append(f"{prefix}WriteStmt({ast_to_str(a, 0).strip()})")
    elif t == 'BinOp':
        return f"({ast_to_str(node['left'], 0).strip()} {node['op']} {ast_to_str(node['right'], 0).strip()})"
    elif t == 'UnaryOp':
        return f"({node['op']}{ast_to_str(node['operand'], 0).strip()})"
    elif t == 'Num':
        return str(node['value'])
    elif t == 'Var':
        return node['name']
    elif t == 'ArrayAccess':
        return f"{ast_to_str(node['array'], 0).strip()}[{ast_to_str(node['index'], 0).strip()}]"
    elif t == 'Call':
        args_str = ', '.join(ast_to_str(a, 0).strip() for a in node['args'])
        return f"{node['func']['name']}({args_str})"
    elif t == 'ExprStmt':
        return ast_to_str(node['expr'], indent)
    else:
